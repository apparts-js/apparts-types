...
const docs = apiToReact(getApi(app));

//// Also available: docs in the open api format, markdown
//// and plain html. See imports.
...
(Only supported by =apiToReact=)

You can add context to your API documentation by structuring it in
sections and adding markdown descriptions to them. Sections can be
nested. They don't have to have any routes in them (e.g. if you need a
chapter "Introduction"), but can. See example below.

The =section= function takes these arguments:
- app :: The express app
- title :: The section title
- description :: (optional) The section description. Markdown is supported.
- routes :: (optional) A function in which you add routes to
  express or add subsections. These sections and routes will be
  displayed with the section.

#+BEGIN_SRC js
const { section } = require("@apparts/types");

section({
  app,
  title: "Introduction",
  description: `
This API doc is for demo and testing purposes.

But to cheer you up, here is a table:
